...
### jitter
A number from 0 to 1 indicating the amount of horizontal jitter to apply when x is not numeric.

```r
clickme(points, jitter = 0,
        x = letters[1:3], y = data.frame("A" = rnorm(10), "B" = rnorm(10), "C" = rnorm(10))) # no jitter

clickme(points, jitter = 0.2,
...
```r
clickme(points, xlim = c(2,6), ylim = c(2, 6),
        x =  1:10, y = 1:10)
```
...
clickme(points, 1:10, file = "some_file_name.html") # generates some_file_name.html in the current working directory

clickme(points, 1:10, file = "some/path/some_file_name.html") # generates some/path/some_file_name.html
```
